public class class693 {
   public static int field7221 = 32768;
   public static int field7222 = 15;
   public static int field7223 = 15;

   class693() throws Throwable {
      throw new Error();
   }

   static void method3877(int var0) {
      try {
         if (class800.field4577 != null && !class800.field4577.method2920((byte)47) && class800.field4577.method2921((byte)1)) {
            class701 var1 = class637.method5936(class643.field9941, class730.field2692.field7765, (byte)88);
            var1.field3364.method6364(class800.field4577.field4651 * 1941368041, 938750212);
            class730.field2692.method4380(var1, (byte)-4);
         }

      } catch (RuntimeException var2) {
         throw class158.method3445(var2, "it.aa(" + ')');
      }
   }

   static final void method3878(class744 var0, int var1) {
      try {
         var0.field3161[(var0.field3156 += -391880689) * 681479919 - 1] = class568.field643 * -1691245391;
      } catch (RuntimeException var3) {
         throw class158.method3445(var3, "it.aml(" + ')');
      }
   }
}
