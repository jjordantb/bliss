public class class150 extends class476 {
   int field5387 = 0;
   int field5388;
   class765 field5389;
   class726 field5390;
   boolean field5391 = true;
   int field5392 = 0;

   boolean method2160() {
      return false;
   }

   void method3376(class848 var1, class879 var2, class135 var3, byte var4) {
      try {
         var2.method6096(var3);
         class933[] var5 = var2.method6089();
         class396[] var6 = var2.method5941();
         if ((this.field5390 == null || this.field5390.field2548) && (var5 != null || var6 != null)) {
            this.field5390 = class726.method1732(Client.field2866 * 443738891, true);
         }

         if (this.field5390 != null) {
            this.field5390.method1731(var1, (long)(443738891 * Client.field2866), var5, var6, false);
            this.field5390.method1727(this.field3639, this.field8299, this.field8297, this.field8296, this.field8298);
         }

      } catch (RuntimeException var7) {
         throw class158.method3445(var7, "akr.i(" + ')');
      }
   }

   public void method3377(byte var1) {
      try {
         if (this.field5390 != null) {
            this.field5390.method1721();
         }

      } catch (RuntimeException var3) {
         throw class158.method3445(var3, "akr.k(" + ')');
      }
   }

   final void method2184() {
      throw new IllegalStateException();
   }

   public class719 method2165(class848 var1, byte var2) {
      return null;
   }

   public class150(class545 var1, int var2, int var3, int var4, int var5, int var6, int var7, int var8, int var9, int var10, int var11, int var12, int var13, boolean var14) {
      super(var1, var4, var5, var6, var7, var8, var9, var10, var11, var12, false, (byte)0);
      this.field5388 = 1921334459 * var2;
      this.field5392 = var13 * -1141067271;
      class745 var15 = class635.field9823.method1053(this.field5388 * -743331725, -2103306753);
      int var16 = 1505778629 * var15.field4107;
      if (var16 != -1) {
         this.field5389 = new class511(this, false);
         int var17 = var15.field4117 ? 0 : 2;
         if (var14) {
            var17 = 1;
         }

         this.field5389.method2780(var16, var3, var17, false, (byte)0);
      }

      this.method2169(1, -656221662);
   }

   public final boolean method3378(byte var1) {
      try {
         return this.field5389 != null && !this.field5389.method2782(-65534);
      } catch (RuntimeException var3) {
         throw class158.method3445(var3, "akr.f(" + ')');
      }
   }

   final boolean method2193() {
      return false;
   }

   class879 method3379(class848 var1, int var2, int var3, int var4) {
      try {
         class745 var5 = class635.field9823.method1053(var3, -1949428738);
         class454 var6 = this.field3638.field3864[this.field3639];
         class454 var7 = this.field3640 < 3 ? this.field3638.field3864[1 + this.field3640] : null;
         class32 var8 = this.method1511().field7637;
         return this.field5389 != null && !this.field5389.method2788(1032843646) ? var5.method2591(var1, var2, true, var6, var7, (int)var8.field5296, (int)var8.field5300, (int)var8.field5299, this.field5389, (byte)2, 345220259) : var5.method2591(var1, var2, true, var6, var7, (int)var8.field5296, (int)var8.field5300, (int)var8.field5299, (class765)null, (byte)2, -2126228993);
      } catch (RuntimeException var9) {
         throw class158.method3445(var9, "akr.p(" + ')');
      }
   }

   void method2164(class848 var1, int var2) {
      try {
         class879 var3 = this.method3379(var1, 0, -743331725 * this.field5388, -1760774279);
         if (var3 != null) {
            this.method3376(var1, var3, this.method1521(), (byte)-58);
         }

      } catch (RuntimeException var4) {
         throw class158.method3445(var4, "akr.bb(" + ')');
      }
   }

   final void method2162(class848 var1, class963 var2, int var3, int var4, int var5, boolean var6, int var7) {
      try {
         throw new IllegalStateException();
      } catch (RuntimeException var9) {
         throw class158.method3445(var9, "akr.bk(" + ')');
      }
   }

   boolean method2157(class848 var1, int var2, int var3, byte var4) {
      return false;
   }

   boolean method2181() {
      return false;
   }

   final boolean method2173(int var1) {
      return false;
   }

   public final void method3380(int var1, int var2) {
      try {
         if (this.field5389 != null && !this.field5389.method2788(-891942946)) {
            this.field5389.method2778(var1, 2083264997);
         }

      } catch (RuntimeException var4) {
         throw class158.method3445(var4, "akr.a(" + ')');
      }
   }

   final void method2205(byte var1) {
      try {
         throw new IllegalStateException();
      } catch (RuntimeException var3) {
         throw class158.method3445(var3, "akr.bq(" + ')');
      }
   }

   public int method2168(int var1) {
      try {
         return 1112786493 * this.field5387;
      } catch (RuntimeException var3) {
         throw class158.method3445(var3, "akr.bm(" + ')');
      }
   }

   boolean method2172() {
      return false;
   }

   final boolean method2194() {
      return false;
   }

   public class719 method2174(class848 var1) {
      return null;
   }

   public class719 method2175(class848 var1) {
      return null;
   }

   class192 method2177(class848 var1) {
      class879 var2 = this.method3379(var1, 2048 | (this.field5392 * -657768375 != 0 ? 5 : 0), -743331725 * this.field5388, -1821357765);
      if (var2 == null) {
         return null;
      } else {
         if (-657768375 * this.field5392 != 0) {
            var2.method6016(1510098944 * this.field5392);
         }

         class135 var3 = this.method1521();
         this.method3376(var1, var2, var3, (byte)-14);
         class192 var4 = class221.method4033(false, 1620653169);
         var2.method5965(var3, this.field3642[0], 0);
         if (this.field5390 != null) {
            class874 var5 = this.field5390.method1729();
            var1.method4866(var5);
         }

         this.field5391 = var2.method5989();
         this.field5387 = var2.method5976() * 2028765461;
         var2.method5973();
         return var4;
      }
   }

   void method2178(class848 var1) {
      class879 var2 = this.method3379(var1, 0, -743331725 * this.field5388, -1484520412);
      if (var2 != null) {
         this.method3376(var1, var2, this.method1521(), (byte)-82);
      }

   }

   void method2180(class848 var1) {
      class879 var2 = this.method3379(var1, 0, -743331725 * this.field5388, -1936222136);
      if (var2 != null) {
         this.method3376(var1, var2, this.method1521(), (byte)-12);
      }

   }

   boolean method2179(class848 var1, int var2, int var3) {
      return false;
   }

   boolean method2192(class848 var1, int var2, int var3) {
      return false;
   }

   boolean method2159(class848 var1, int var2, int var3) {
      return false;
   }

   final void method2182(class848 var1, class963 var2, int var3, int var4, int var5, boolean var6) {
      throw new IllegalStateException();
   }

   class192 method2201(class848 var1, int var2) {
      try {
         class879 var3 = this.method3379(var1, 2048 | (this.field5392 * -657768375 != 0 ? 5 : 0), -743331725 * this.field5388, -1982313638);
         if (var3 == null) {
            return null;
         } else {
            if (-657768375 * this.field5392 != 0) {
               var3.method6016(1510098944 * this.field5392);
            }

            class135 var4 = this.method1521();
            this.method3376(var1, var3, var4, (byte)-52);
            class192 var5 = class221.method4033(false, 1486074028);
            var3.method5965(var4, this.field3642[0], 0);
            if (this.field5390 != null) {
               class874 var6 = this.field5390.method1729();
               var1.method4866(var6);
            }

            this.field5391 = var3.method5989();
            this.field5387 = var3.method5976() * 2028765461;
            var3.method5973();
            return var5;
         }
      } catch (RuntimeException var7) {
         throw class158.method3445(var7, "akr.bo(" + ')');
      }
   }

   boolean method2176() {
      return this.field5391;
   }

   final void method2185() {
      throw new IllegalStateException();
   }

   boolean method2183(short var1) {
      try {
         return this.field5391;
      } catch (RuntimeException var3) {
         throw class158.method3445(var3, "akr.be(" + ')');
      }
   }

   public int method2187() {
      return 1112786493 * this.field5387;
   }

   public int method2188() {
      return 1112786493 * this.field5387;
   }

   public int method2186() {
      return 1112786493 * this.field5387;
   }

   boolean method2189() {
      return this.field5391;
   }

   boolean method2156() {
      return this.field5391;
   }

   boolean method2158() {
      return this.field5391;
   }

   final boolean method2191() {
      return false;
   }

   final boolean method2207() {
      return false;
   }

   final void method2195(class848 var1, class963 var2, int var3, int var4, int var5, boolean var6) {
      throw new IllegalStateException();
   }

   boolean method2190() {
      return this.field5391;
   }

   public final boolean method3381(int var1) {
      try {
         return this.field5389 == null || this.field5389.method2788(1776258671);
      } catch (RuntimeException var3) {
         throw class158.method3445(var3, "akr.b(" + ')');
      }
   }

   boolean method2206(byte var1) {
      return false;
   }
}
