public class class188 {
   public String field5347;
   public int field5348;

   static final void method3343(class744 var0, byte var1) {
      try {
         int var2 = var0.field3161[--var0.field3156];
         class564 var3 = class449.method3756(var2, (byte)-11);
         class131 var4 = class382.field1410[var2 >> 16];
         class827.method5356(var3, var4, var0, -61919931);
      } catch (RuntimeException var5) {
         throw class158.method3445(var5, "os.ho(" + ')');
      }
   }

   static final void method3344(class744 var0, byte var1) {
      try {
         var0.field3161[++var0.field3156 - 1] = class881.field10181;
      } catch (RuntimeException var3) {
         throw class158.method3445(var3, "os.aha(" + ')');
      }
   }

   static void method3345(class744 var0, int var1) {
      try {
         var0.field3157[++var0.field3158 - 1] = class735.field3009.method4224(var0.field3161[--var0.field3156], 245040087).field9002;
      } catch (RuntimeException var3) {
         throw class158.method3445(var3, "os.a(" + ')');
      }
   }

   static int method3346(int var0, int var1) {
      try {
         byte var2;
         if (var0 > 12010) {
            class115.method1276(976671472);
            var2 = 4;
         } else if (var0 > 5033) {
            class846.method5123((short)16862);
            var2 = 3;
         } else if (var0 > 2018) {
            class671.method4234(-476807021);
            var2 = 2;
         } else {
            class874.method5851(true, -930644451);
            var2 = 1;
         }

         if (class615.field8903.field9137.method3689(-1158457222) != 2) {
            class615.field8903.method5391(class615.field8903.field9136, 2, 1051550423);
            class337.method77(2, false, 622850291);
         } else {
            class615.field8903.method5390(class615.field8903.field9137, true, -1871094786);
         }

         class95.method523(656179282);
         return var2;
      } catch (RuntimeException var3) {
         throw class158.method3445(var3, "os.i(" + ')');
      }
   }
}
