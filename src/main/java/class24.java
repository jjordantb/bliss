public class class24 {
   public int field6482;
   public class282 field6483;
   public int[] field6484;

   static final void method3463(class744 var0, byte var1) {
      try {
         int var2 = var0.field3161[(var0.field3156 -= -391880689) * 681479919];
         class564 var3 = class449.method3756(var2, (byte)90);
         class131 var4 = class382.field1410[var2 >> 16];
         class866.method5840(var3, var4, var0, 1938124725);
      } catch (RuntimeException var5) {
         throw class158.method3445(var5, "hj.kg(" + ')');
      }
   }

   static final void method3464(class744 var0, int var1) {
      try {
         var0.field3161[(var0.field3156 += -391880689) * 681479919 - 1] = class615.field8903.field9126.method6339(-1807205533) == 1 ? 1 : 0;
      } catch (RuntimeException var3) {
         throw class158.method3445(var3, "hj.ajs(" + ')');
      }
   }

   static final void method3465(class744 var0, int var1) {
      try {
         int var2 = var0.field3161[(var0.field3156 -= -391880689) * 681479919];
         if (class382.field1410[var2] == null) {
            var0.field3161[(var0.field3156 += -391880689) * 681479919 - 1] = 0;
         } else {
            var0.field3161[(var0.field3156 += -391880689) * 681479919 - 1] = class382.field1410[var2].field1103.length;
         }

      } catch (RuntimeException var3) {
         throw class158.method3445(var3, "hj.alg(" + ')');
      }
   }
}
