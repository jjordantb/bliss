public class class850 extends class48 implements class297, class813 {
    boolean field8839 = false;
    long field8840;

    class850(class955 var1, int[] var2, int var3, int var4, int var5, int var6, boolean var7) {
        this.RA(var1, var2, var3, var4, var5, var6, var7);
    }

    class850(class955 var1, int var2, int var3, int var4, int var5, boolean var6) {
        this.K(var1, var2, var3, var4, var5, var6);
    }

    class850(class955 var1, int[] var2, byte[] var3, byte[] var4, int var5, int var6, int var7, int var8) {
        this.EA(var1, var2, var3, var4, var5, var6, var7, var8);
    }

    class850(class955 var1, int var2, int var3, boolean var4) {
        this.field8839 = var4;
        this.UA(var1, var2, var3);
    }

    public void method6550(boolean var1) {
        this.M(this.field8840, var1);
    }

    native void F(long var1, int[] var3);

    native void UA(class955 var1, int var2, int var3);

    native void EA(class955 var1, int[] var2, byte[] var3, byte[] var4, int var5, int var6, int var7, int var8);

    native void ea(long var1, int var3, int var4, int var5, int var6, int var7, int var8);

    public int method2344() {
        return this.GA(this.field8840);
    }

    native void M(long var1, boolean var3);

    public void method3104(int var1, int var2, int var3, int var4) {
        this.U(this.field8840, var1, var2, var3, var4);
    }

    native void U(long var1, int var3, int var4, int var5, int var6);

    public void method3105(int[] var1) {
        this.F(this.field8840, var1);
    }

    native int da(long var1);

    native int WA(long var1);

    public void method2755() {
    }

    public int method3108() {
        return this.GA(this.field8840);
    }

    native int ba(long var1);

    public int method2343() {
        return this.da(this.field8840);
    }

    public void method3158(int var1, int var2, int var3, int var4, int var5, int var6) {
        this.ea(this.field8840, var1, var2, var3, var4, var5, var6);
    }

    public void method3115(int var1, int var2, int var3) {
        this.j(this.field8840, var1, var2, var3);
    }

    native void j(long var1, int var3, int var4, int var5);

    native void Y(long var1, int var3, int var4, int var5, int var6, int var7);

    public void method3137(int var1, int var2, class23 var3, int var4, int var5) {
        this.o(this.field8840, var1, var2, ((class280) var3).field10644, var4, var5);
    }

    native void o(long var1, int var3, int var4, long var5, int var7, int var8);

    native void aa(long var1, int var3, int var4, int var5, int var6, int var7, int var8, int var9);

    public void method3145(int var1, int var2, int var3, int var4, int var5, int var6, int var7) {
        this.aa(this.field8840, var1, var2, var3, var4, var5, var6, var7);
    }

    void method3125(float var1, float var2, float var3, float var4, float var5, float var6, int var7, int var8, int var9, int var10) {
        this.e(this.field8840, var1, var2, var3, var4, var5, var6, var7, var8, var9, var10);
    }

    native void ha(long var1, int var3, int var4, int var5, int var6, int var7, int var8, int var9, int var10);

    void method3127(float var1, float var2, float var3, float var4, float var5, float var6, int var7, class23 var8, int var9, int var10) {
        this.FA(this.field8840, var1, var2, var3, var4, var5, var6, var7, ((class280) var8).field10644, var9, var10, this.field8839);
    }

    native void FA(long var1, float var3, float var4, float var5, float var6, float var7, float var8, int var9, long var10, int var12, int var13, boolean var14);

    public void method3114(int var1, int var2, int var3, int var4, int var5) {
        this.Y(this.field8840, var1, var2, var3, var4, var5);
    }

    public int method5594() {
        return this.WA(this.field8840);
    }

    void method3118(int var1, int var2, int var3, int var4, int var5, int var6, int var7, int var8) {
        this.ha(this.field8840, var1, var2, var3, var4, var5, var6, var7, var8);
    }

    public class813 method3110() {
        return this.field8839 ? this : null;
    }

    native int GA(long var1);

    public int method3106() {
        return this.da(this.field8840);
    }

    native void RA(class955 var1, int[] var2, int var3, int var4, int var5, int var6, boolean var7);

    native void e(long var1, float var3, float var4, float var5, float var6, float var7, float var8, int var9, int var10, int var11, int var12);

    public int method3109() {
        return this.ba(this.field8840);
    }

    native void K(class955 var1, int var2, int var3, int var4, int var5, boolean var6);
}
