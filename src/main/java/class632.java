public class class632 extends class666 implements class920 {
   class482 field9776;

   public boolean method1977(int var1, int var2, long var3) {
      return super.method1977(var1, var2, var3);
   }

   class482 method5849() {
      return this.field9776;
   }

   public void method5738() {
      super.method5738();
   }

    @Override
    public void method2755() {

    }

    public void method6489(int var1) {
      super.method5735(var1 * this.field9776.field8406);
   }

   class632(class608 var1, class482 var2, boolean var3) {
      super(var1, 34963, var3);
      this.field9776 = var2;
   }

   public long method1976(int var1, int var2) {
      return super.method1976(var1, var2);
   }

   public int method1974() {
      return super.method1974();
   }

   public void method1983() {
      super.method1983();
   }
}
