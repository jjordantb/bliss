public class class679 {
   public static int field7516 = 48;
   public static int field7517 = -3;
   public static int field7518 = -4;
   public static int field7519 = 3;
   public static int field7520 = 1;
   public static int field7521 = 7;
   public static int field7522 = -2;
   public static int field7523 = 6;
   public static int field7524 = 2;
   public static int field7525 = 35;
   public static int field7526 = 15;
   public static int field7527 = 21;
   public static int field7528 = 45;
   public static int field7529 = 29;
   public static int field7530 = 23;
   public static int field7531 = 42;
   public static int field7532 = -5;
   public static int field7533 = 9;

   class679() throws Throwable {
      throw new Error();
   }

   static final void method4266(class744 var0, int var1) {
      try {
         int var2 = var0.field3161[--var0.field3156];
         Widget var3 = class449.method3756(var2, (byte)2);
         WidgetContainer var4 = class382.widgetContainers[var2 >> 16];
         class569.method11(var3, var4, var0, -16777216);
      } catch (RuntimeException var5) {
         throw class158.method3445(var5, "ow.kf(" + ')');
      }
   }

   static final void method4267(class744 var0, int var1) {
      try {
         class237 var2 = var0.field3178 ? var0.field3164 : var0.field3163;
         Widget var3 = var2.field8255;
         WidgetContainer var4 = var2.field8254;
         class649.method5545(var3, var4, var0, 896700904);
      } catch (RuntimeException var5) {
         throw class158.method3445(var5, "ow.ey(" + ')');
      }
   }

   public static int method4268(int var0) {
      try {
         if (class200.field7759 == -1) {
            class200[] var1 = class176.method3170((byte)-111);

            for(int var2 = 0; var2 < var1.length; ++var2) {
               class200 var3 = var1[var2];
               if (var3.field7758 > class200.field7759) {
                  class200.field7759 = var3.field7758;
               }
            }

            ++class200.field7759;
         }

         return class200.field7759;
      } catch (RuntimeException var4) {
         throw class158.method3445(var4, "ow.b(" + ')');
      }
   }

   static final void method4269(class744 var0, int var1) {
      try {
         class237 var2 = var0.field3178 ? var0.field3164 : var0.field3163;
         Widget var3 = var2.field8255;
         int var4 = -1;
         int var5 = -1;
         class118 var6 = var3.method826(class593.field1623, 193353438);
         if (var6 != null) {
            var4 = var6.field2152;
            var5 = var6.field2153;
         }

         var0.field3161[++var0.field3156 - 1] = var4;
         var0.field3161[++var0.field3156 - 1] = var5;
      } catch (RuntimeException var7) {
         throw class158.method3445(var7, "ow.py(" + ')');
      }
   }

   static final void method4270(Widget var0, WidgetContainer var1, class744 var2, int var3) {
      try {
         String var4 = (String)var2.field3157[--var2.field3158];
         int[] var5 = class960.method2212(var4, var2, -1668247696);
         if (var5 != null) {
            var4 = var4.substring(0, var4.length() - 1);
         }

         var0.field902 = class634.method5872(var4, var2, -2046058202);
         var0.field913 = var5;
         var0.field963 = true;
      } catch (RuntimeException var6) {
         throw class158.method3445(var6, "ow.lm(" + ')');
      }
   }

   public static final int method4271(int var0, int var1, int var2, int var3) {
      try {
         if (class730.field2697.method5317(-1611682495) == null) {
            return 0;
         } else {
            int var4 = var0 >> 9;
            int var5 = var1 >> 9;
            if (var4 >= 0 && var5 >= 0 && var4 <= class730.field2697.method5271(-1956967905) - 1 && var5 <= class730.field2697.method5272(1920772834) - 1) {
               int var6 = var2;
               if (var2 < 3 && (class730.field2697.method5296(2001279106).field5406[1][var4][var5] & 2) != 0) {
                  var6 = var2 + 1;
               }

               return class730.field2697.method5317(-1611682495).field3862[var6].method3887(var0, var1, -1909964281);
            } else {
               return 0;
            }
         }
      } catch (RuntimeException var7) {
         throw class158.method3445(var7, "ow.jx(" + ')');
      }
   }

   static final void method4272(class744 var0, int var1) {
      try {
         int var2 = var0.field3161[--var0.field3156];
         Widget var3 = class449.method3756(var2, (byte)0);
         var0.field3161[++var0.field3156 - 1] = var3.field973;
      } catch (RuntimeException var4) {
         throw class158.method3445(var4, "ow.rg(" + ')');
      }
   }

   static final void method4273(class744 var0, int var1) {
      try {
         var0.field3161[++var0.field3156 - 1] = class730.field2624 ? 1 : 0;
      } catch (RuntimeException var3) {
         throw class158.method3445(var3, "ow.uk(" + ')');
      }
   }
}
